...
function register_mysettings() {
	...
	register_setting( 'wpsh-settings-group', 'wpsh_enable' );
	register_setting( 'wpsh-settings-group', 'wpsh_bg_color' );
	register_setting( 'wpsh-settings-group', 'wpsh_text_color' );
	register_setting( 'wpsh-settings-group', 'wpsh_close_icon_color' );
...
<form method="post" action="options.php">
    ...
    <table class="form-table">
    	<tr valign="top">
        	...
        	<td><input type="checkbox" name="wpsh_enable" value="1" <?php if(esc_attr( get_option('wpsh_enable'))) echo "checked"; ?> /> &nbsp;&nbsp;Enable/Disable the notification</td>
        </tr>
        ...
        <td><input type="text" name="wpsh_close_icon_color" class="color-field" value="<?php echo esc_attr( get_option('wpsh_close_icon_color') ); ?>" /> &nbsp;&nbsp;Close Icon color of the header</td>
        </tr>
...
function show_wpsh_header(){
	$is_enabled = esc_attr(get_option('wpsh_enable'));
...
function wpsh_custom_styles(){ ?>
	...
	<?php if(esc_attr(get_option('wpsh_closable')) == 1) { ?>
		.wpsh_close{
			color:<?php echo esc_attr( get_option('wpsh_close_icon_color') ); ?>;;
			opacity:0.5;
			float:right;
			margin-right:30px;
			font-size:14px;
...
function wpsh_plugin_update() { 
	...
	if(!get_option('wpsh_enable')){
		add_option('wpsh_enable','1');
	}
	if(!get_option('wpsh_close_icon_color')){
		add_option('wpsh_close_icon_color','#FFF');
	}
}
